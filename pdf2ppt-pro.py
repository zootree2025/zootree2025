...
class PDFToPPTConverter:
    def __init__(self, root: TkinterDnD.Tk):
        ...
        self.root.configure(bg="#C19A6B")  # 大地色背景
    ...
    def _create_widgets(self):
        ...
        # --- PDF 輸入區 ---
        pdf_frame = tk.Frame(self.root, bg="#C19A6B")
        pdf_frame.pack(pady=15)
        tk.Label(pdf_frame, text="PDF 文件:", bg="#C19A6B", font=font_setting).pack(side=tk.LEFT)
        self.pdf_entry = tk.Entry(pdf_frame, width=40, font=font_setting)
        self.pdf_entry.pack(side=tk.LEFT, padx=10)
        tk.Button(pdf_frame, text="瀏覽", font=font_setting, command=self.select_pdf, relief="flat").pack(side=tk.LEFT)

        # --- PPT 儲存區 ---
        ppt_frame = tk.Frame(self.root, bg="#C19A6B")
        ppt_frame.pack(pady=15)
        tk.Label(ppt_frame, text="PPT 輸出:", bg="#C19A6B", font=font_setting).pack(side=tk.LEFT)
        self.ppt_entry = tk.Entry(ppt_frame, width=40, font=font_setting)
        self.ppt_entry.pack(side=tk.LEFT, padx=10)
        tk.Button(ppt_frame, text="儲存", font=font_setting, command=self.select_save_location, relief="flat").pack(side=tk.LEFT)

        # --- 比例選單 ---
        ratio_frame = tk.Frame(self.root, bg="#C19A6B")
        ratio_frame.pack(pady=10)
        tk.Label(ratio_frame, text="選擇PPT比例:", bg="#C19A6B", font=font_setting).pack()
        ...
        self.convert_btn = tk.Button(self.root, text="開始轉換", font=font_setting,
                                     command=self.start_conversion, relief="flat", width=20, height=2)
        ...
        # --- 動畫提示 ---
        self.anim_label = tk.Label(self.root, text="", bg="#C19A6B", font=("微軟正黑體", 11))
    ...
    def select_save_location(self):
        ...
        filename = filedialog.asksaveasfilename(
            ...
            initialfile=f"{os.path.splitext(os.path.basename(self.pdf_path))[0]}.pptx",
